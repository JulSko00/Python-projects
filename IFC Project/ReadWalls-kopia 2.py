...
class IfcView:
    ...
    def display_area_report(self, spaces):
        ...
        if spaces is None:
            self.result_label.config(text="No file selected or file could not be opened.")
            return
        self.result_label.config(text=f"Total number of spaces with net floor area: {len(spaces)}")
    ...
    def display_volumes(self, volumes):
        ...
        if volumes is None:
            self.result_label.config(text="No file selected or file could not be opened.")
            return
        self.result_label.config(text=f"Total number of spaces with volume: {len(volumes)}")
